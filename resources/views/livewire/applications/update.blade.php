@if(($application->statuses->reference != 'application-submitted') && ($application->statuses->reference != 'application-accepted')
&& ($application->statuses->reference != 'application-revision') && ($application->statuses->reference != 'application-author-revised'))
    <x-organisms.modal
        showButton={{true}}
        ...
        buttonSubmitName="Update Application"
        passingData=""
        custom=""
        xdata="
             pid:'{{$application->id}}',
            sid:'0',
            service:'{{$application->services_id}}',
            passport:'{{$application->passports_id}}',
            university:'{{$application->universities_id}}'
    "
    >
        <x-molecules.modal.content>
            <div
                class="row p-2 justify-content-start align-items-center mx-0 border rounded-3 mt-2 mb-4 bg-light-primary">
                ...
                <div class="col-1 pe-0 pb-2 pt-2" style="max-width: 70px"><h4
                        class="text-gray-400 m-0 font-semibold">{{$currentPage+1}}/{{count($multiStep)}}</h4></div>
            ...
            @switch($currentPage)
                ...
                <x-molecules.form.custom-form
                    showButton={{true}}
                        id="{{$application->id}}_{{$currentPage}}"
                ...
                >
                    <x-atoms.bootstrap.input-select-livewire
                        ...
                        id="service_update_{{$application->id}}"
                        label="Service"
                    ...
                    <div class="form-check form-check-inline d-flex justify-content-start align-items-center p-0 mt-3">
                        <input class="form-check-input d-flex m-0 p-0 me-2" type="checkbox" value=""
                               id="terms_checkbox_{{$application->id}}"
                               wire:model="terms">
                    ...
                    <x-atoms.div class="row" custom="">
                        <x-atoms.div class="col-12 pt-4 pb-3" custom="">
                            ...
                            <button type="submit" class="btn btn-secondary float-end"
                                    wire:key="application_service_create_modal_update_submit_spinner_{{$application->id}}_{{$currentPage}}"
                                    wire:click.prevent="submit_application_service_create"
                            >Next
                            </button>
                        </x-atoms.div>
                    </x-atoms.div>
                ...
                <x-molecules.form.custom-form
                    showButton={{true}}
                        id="{{$application->id}}_{{$currentPage}}"
                    name="application_passport_update"
                ...
                >
                    <x-atoms.bootstrap.input-select-livewire
                        ...
                        id="passport_update_{{$application->id}}"
                        label="Passport"
                    ...
                    <x-atoms.div class="row" custom="">
                        <x-atoms.div class="col-12 pt-4 pb-3" custom="">
                            <button class="btn btn-secondary float-start" wire:click.prevent="previousPage">Back
                            </button>
                            <button type="submit" class="btn btn-secondary float-end"
                                    wire:key="application_passport_create_modal_update_submit_spinner_{{$application->id}}_{{$currentPage}}"
                                    wire:click.prevent="submit_application_passport_create">Next
                            </button>
                        </x-atoms.div>
                    </x-atoms.div>
                ...
                >
                    <x-atoms.bootstrap.input-select-livewire
                        ...
                        id="university_update_{{$application->id}}"
                        label="University"
                    ...
                    <x-atoms.div class="row" custom="">
                        <x-atoms.div class="col-12 pt-4 pb-3" custom="">
                            ...
                            <button type="submit" class="btn btn-secondary float-end"
                                    wire:key="application_create_modal_update_submit_spinner_{{$application->id}}_{{$currentPage}}"
                                    wire:click.prevent="nextPage">Next
                            </button>
                        </x-atoms.div>
                    </x-atoms.div>
                </x-molecules.form.custom-form>
                                                ...
                                                @if(in_array($requirement->id,json_decode($documents->documents->pluck('service_requirements_id'),true)))
                                                    @foreach($documents->documents as $document)
                                                        @if($document->service_requirements_id ==$requirement->id)
                                                            <img
                                                                src="{{$document->getFirstMedia('documents')->getUrl() }}"
...
{{--                                                        imagePreviewHeight: 170,--}}
                                        ...
                                        <x-atoms.div class="row pt-3 justify-content-center align-items-center"
                                            ...
                                            <div
                                                ...
                                                <button
                                                    type="submit"
                                                    {{--wire:click.prevent="submit_document_update({{$requirement->id}})"--}}
                    ...
                    >
                        ...
                        <x-atoms.div class="row" custom="">
                            <x-atoms.div class="col-12 pt-4 pb-3" custom="">
                                ...
                                <button type="submit" class="btn btn-secondary float-end"
                                        wire:key="application_create_modal_update_submit_spinner_0_{{$application->id}}_{{$currentPage}}">
                                    Next
                                </button>
                            </x-atoms.div>
                        </x-atoms.div>
                ...
                >
                    <x-atoms.div class="bg-light p-3 mb-4 rounded-3 border" custom="">
                        <h6 class="text-danger fw-normal p-0 m-0"><i class="fa-solid fa-circle-exclamation"></i> Review
                            your application carefully before you submit. Use the back button to go to the right section
                            inorder to make changes!</h6>
                    </x-atoms.div>
        ...
        <x-molecules.modal.content>
            <x-atoms.div class="" custom="">
                ...
                <p>Certain areas of On are restricted from being access by you and On may further restrict access by you
                    to
                    any areas of On, at any time, in absolute discretion. Any user ID and password you may have for On
                    are
...
@else
    ...
    >
        <x-molecules.modal.content>
            ...
            <x-atoms.div class="row p-2" custom="">
                <x-atoms.div class="col-4 font-bold" custom="">Submitted By</x-atoms.div>
                <x-atoms.div class="col-8" custom=""> {{$application->user->name}} </x-atoms.div>
            </x-atoms.div>

            <x-atoms.div class="row p-2" custom="">
                <x-atoms.div class="col-4 font-bold" custom="">Service Required</x-atoms.div>
                <x-atoms.div class="col-8" custom=""> {{$application->services->name}} </x-atoms.div>
            </x-atoms.div>

            <x-atoms.div class="row p-2" custom="">
                <x-atoms.div class="col-4 font-bold" custom="">University</x-atoms.div>
                <x-atoms.div class="col-8" custom=""> {{$application->universities->name}} </x-atoms.div>
            </x-atoms.div>

            <x-atoms.div class="row p-2" custom="">
                <x-atoms.div class="col-4 font-bold" custom="">Passport</x-atoms.div>
                <x-atoms.div class="col-8" custom=""> {{$application->passports->passport_number}} </x-atoms.div>
            </x-atoms.div>

            <h5 class="pb-0 mb-0 pt-4">Supporting documents</h5>
            <hr>
            @isset($documents)
                @foreach($documents->documents as $document)
                    {{--                    <button wire:click.prevent="document_file({{$document}})" class="btn btn-sm btn-primary">{{$document->service_requirements->name}}</button>--}}

                    <x-atoms.div class="row pb-2" custom="">
                        <x-atoms.div class="col-3" custom="">
                            <a class="btn
                              @if($document->accepted) btn-success @elseif($document->rejected) btn-danger @else btn-primary @endif
                                btn-sm" role="button" href="{{$document->getFirstMedia('documents')->getUrl() }}"
                               download="{{$document->service_requirements->name}}">
                                @if($document->accepted)<i
                                    class="fa-solid fa-circle-check"></i> @elseif($document->rejected) <i
                                    class="fa-solid fa-circle-xmark"></i> @endif
                                {{$document->service_requirements->name}}
                            </a>
                        </x-atoms.div>
                        <x-atoms.div class="col" custom="">
                            @if($application->statuses->reference === 'application-accepted')
                                <span
                                    class="badge bg-primary"> Accepted {{\Carbon\Carbon::parse($application->updated_at)->diffForhumans()}}</span>

                            @else
                                @if(!$document->accepted)

                                    <button wire:click.prevent="accept_document({{$document->id}})"
                                            class="btn btn-sm btn-success rounded-pill ps-2 pe-3 d-inline"><i
                                            class="fa-solid fa-circle-check"></i> Accept
                                    </button>
                                @endif

                                @if(!$document->rejected)
                                    <button wire:click.prevent="reject_document({{$document->id}})"
                                            class="btn btn-sm btn-danger rounded-pill ps-2 pe-3 d-inline"><i
                                            class="fa-solid fa-circle-xmark"></i> Reject
                                    </button>
                                @endif
                            @endif
                    ...
                    <x-molecules.form.custom-form
                        showButton={{true}}
                            id="{{$document->id}}"
                        name="document_select_update"
                        buttonSubmitName="Update Image"
                        footerClass="d-none"
                    >
                            ...
                            >
                                <x-atoms.div class="row" custom="">
                                    <x-atoms.div class="col-2" custom="">
                                        <img
                                            src="{{$document->getFirstMedia('documents')->getUrl() }}"
                                            class="img-fluid img-thumbnail mb-0" alt="">
                                    </x-atoms.div>
                                    <x-atoms.div class="col-10" custom="">
                                        <x-atoms.bootstrap.input
                                            {{--                                        wire:model="files.file_{{$requirement->id}}"--}}
                                            container=""
                                            id="{{$document->id}}"
                                            size=""
                                            name="files.revised_file_{{$document->id}}"
                                            label=""
                                            type="file"
                                            placeholder="File"
                                            value=""
...
{{--                                                        imagePreviewHeight: 170,--}}
                                ...
                                <x-atoms.div class="row pt-3 justify-content-center align-items-center"
                                    ...
                                    <div
                                        ...
                                        <button
                                            type="submit"
                                            wire:click.prevent="submit_document_select_update({{$document->id}})"
                                            class="btn btn-sm btn-secondary d-inline rounded-5 text-xs px-3 py-1">
                                            Update File
                                        </button>
                                    </div>
